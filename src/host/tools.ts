...
function failure(code: ControllerCode, message: string): BaseResult {
  return { ok: false, code, message }
}

function success(code: ControllerCode, message: string): BaseResult {
...
function outcomeResult<V extends object>(
  ...
  if (outcome.ack !== null) {
    if (outcome.ack.ok) {
      return {
        ...success((outcome.ack.code as ControllerCode | undefined) ?? 'OK', outcome.ack.message ?? okMessage),
        delivered: true,
        queued: false,
        ...value,
      }
    ...
    return {
      ...failure((outcome.ack.code as ControllerCode | undefined) ?? 'INTERNAL_ERROR', outcome.ack.message ?? '客户端执行失败。'),
      delivered: true,
      queued: false,
      ...value,
    }
...
function textOf(result: BaseResult & Record<string, unknown>): string {
  ...
  if (result.path !== undefined && typeof result.path === 'string') lines.push(`路径: ${result.path}`)
  if (result.sidebarVisible !== undefined) lines.push(`侧边栏显示: ${String(result.sidebarVisible)}`)
  if (result.currentFile !== undefined) lines.push(`当前文件: ${String(result.currentFile)}`)
  if (result.previousFile !== undefined) lines.push(`上一个文件: ${String(result.previousFile)}`)
  if (result.queued === true) lines.push('（操作已排队，将在 Sidebar 可见时自动应用）')
  return lines.join('\n')
}
  ...
  oneOf: [
    {
      ...
      properties: {
        ...
        updatedAt: { type: 'number' as const, required: true as const, description: '最近状态更新时间（epoch ms）。' },
      },
    },
    { type: 'null' as const },
  ],
...
const entrySchema = {
  ...
  additionalProperties: false as const,
  properties: {
    name: { type: 'string' as const, required: true as const, description: '条目名。' },
    path: { type: 'string' as const, required: true as const, description: '条目绝对路径。' },
    isDir: { type: 'boolean' as const, required: true as const, description: '是否目录。' },
    hidden: { type: 'boolean' as const, required: true as const, description: '是否隐藏（点开头）。' },
    isSymlink: { type: 'boolean' as const, required: true as const, description: '是否符号链接。' },
    broken: { type: 'boolean' as const, required: true as const, description: '符号链接目标是否失效。' },
  },
} satisfies ParameterPropertySpec
...
export function stateView(state: ControllerState | null, connected: boolean): ControllerStateView {
  if (state === null) {
...
export function registerControllerTools(ctx: { tools: { register(tool: unknown): () => void } }, deps: ControllerDeps): () => void {
  const disposers: Array<() => void> = []

  const getCwdOrError = async (sessionId: string): Promise<{ cwd: string } | { error: BaseResult }> => {
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    description:
      '获取当前 Sidebar / 工作区状态：侧边栏是否显示、当前文件、上一个文件、已打开文件、已展开文件夹、工作区根目录。'
    ...
    output: {
      ...
      render: (_args, value) => [{ type: 'text', text: renderState(value as never) }],
    },
  ...
  const sidebarVisibilityTool = (name: 'show_sidebar' | 'hide_sidebar', visible: boolean): void => {
    disposers.push(ctx.tools.register(defineTool({
      name,
      ...
      output: {
        schema: outputWith({
          sidebarVisible: { type: 'boolean' as const, description: '操作后的侧边栏显示状态。' },
          delivered: { type: 'boolean' as const, description: '是否已送达 Sidebar 客户端。' },
          queued: { type: 'boolean' as const, description: '是否已排队等待 Sidebar 可见后应用。' },
        }),
      ...
      execute: async (_args, exec) => {
        ...
        return outcomeResult(
          outcome,
          visible ? '侧边栏已打开。' : '侧边栏已关闭。',
          visible ? '侧边栏当前未连接，打开操作已排队，将在 Sidebar 可见时自动应用。' : '侧边栏当前未连接，关闭操作已排队，将在 Sidebar 可见时自动应用。',
  ...
  disposers.push(ctx.tools.register(defineTool({
    name: 'list_files',
    description:
      '列出目录内容（单层）。path 省略时列出工作区根目录。适合「看看有哪些文件」「列一下 docs 目录」。'
    ...
    output: {
      schema: outputWith({
        path: { type: 'string' as const, description: '被列出的目录绝对路径。' },
        entries: entriesSchema,
        truncated: { type: 'boolean' as const, description: '条目是否因超出上限被截断。' },
      }),
      render: (_args, value) => [{ type: 'text', text: renderListing(value as never) }],
    },
  ...
  const folderTool = (name: 'expand_folder' | 'collapse_folder'): void => {
    disposers.push(ctx.tools.register(defineTool({
      name,
      ...
      output: {
        schema: outputWith({
          path: { type: 'string' as const, description: '目标文件夹绝对路径。' },
          expanded: { type: 'boolean' as const, description: '操作后该文件夹是否处于展开状态。' },
          delivered: { type: 'boolean' as const, description: '是否已送达 Sidebar 客户端。' },
          queued: { type: 'boolean' as const, description: '是否已排队。' },
        }),
        render: (_args, value) => [{ type: 'text', text: textOf(value as never) }],
      },
      execute: async (args, exec) => {
        ...
        return outcomeResult(
          outcome,
          expanded ? '文件夹已展开。' : '文件夹已收起。',
          expanded ? '文件夹展开操作已排队，将在 Sidebar 可见时自动应用。' : '文件夹收起操作已排队，将在 Sidebar 可见时自动应用。',
          { path: resolved.path, expanded },
        )
      },
    })))
  }
  ...
  disposers.push(ctx.tools.register(defineTool({
    name: 'refresh_tree',
    description:
      '刷新文件树：重新读取工作区根目录并让 Sidebar 重新加载已展开的目录。'
      + '适合「刷新文件树」「重新看看文件」等场景（例如外部新建了文件）。',
    parameters: {},
    output: {
      schema: outputWith({
        rootPath: { type: 'string' as const, description: '工作区根目录绝对路径。' },
        entries: entriesSchema,
        truncated: { type: 'boolean' as const, description: '根目录条目是否被截断。' },
        delivered: { type: 'boolean' as const, description: '是否已送达 Sidebar 客户端。' },
        queued: { type: 'boolean' as const, description: '是否已排队。' },
      }),
      render: (_args, value) => [{ type: 'text', text: renderListing(value as never) }],
  ...
  disposers.push(ctx.tools.register(defineTool({
    name: 'open_file',
    description:
      '在侧边栏中打开一个文件（若已打开则激活它）。path 为相对工作区根目录的相对路径或绝对路径。'
    ...
    output: {
      schema: outputWith({
        path: { type: 'string' as const, description: '被打开文件的绝对路径。' },
        delivered: { type: 'boolean' as const, description: '是否已送达 Sidebar 客户端。' },
        queued: { type: 'boolean' as const, description: '是否已排队。' },
      }),
      render: (_args, value) => [{ type: 'text', text: textOf(value as never) }],
  ...
  disposers.push(ctx.tools.register(defineTool({
    name: 'close_file',
    description:
      '关闭一个已打开的文件（关闭其标签页）。path 省略时关闭当前文件。'
      + '适合「关闭当前文件」「把 A.md 关掉」。',
    parameters: {
      path: {
        type: 'string',
    ...
    output: {
      schema: outputWith({
        path: { type: 'string' as const, description: '被关闭文件的绝对路径（关闭当前文件时必有）。' },
        delivered: { type: 'boolean' as const, description: '是否已送达 Sidebar 客户端。' },
        queued: { type: 'boolean' as const, description: '是否已排队。' },
      }),
      render: (_args, value) => [{ type: 'text', text: textOf(value as never) }],
  ...
  disposers.push(ctx.tools.register(defineTool({
    name: 'activate_file',
    description:
      '切换到（激活）一个已打开的文件标签页；文件必须已打开（未打开请用 open_file）。'
      + '适合「切到项目计划」「切到那个文件」。',
    parameters: {
      path: {
        type: 'string',
    ...
    output: {
      schema: outputWith({
        path: { type: 'string' as const, description: '目标文件绝对路径。' },
        delivered: { type: 'boolean' as const, description: '是否已送达 Sidebar 客户端。' },
        queued: { type: 'boolean' as const, description: '是否已排队。' },
      }),
      render: (_args, value) => [{ type: 'text', text: textOf(value as never) }],
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    description:
      '重新打开 / 回到上一个文件（previous_file，即本次当前文件之前的那个文件）。'
      + '适合「回到刚才那个文件」「切回上一个文件」。',
    parameters: {},
    output: {
      schema: outputWith({
        path: { type: 'string' as const, description: '被重新打开的上一个文件绝对路径。' },
        delivered: { type: 'boolean' as const, description: '是否已送达 Sidebar 客户端。' },
        queued: { type: 'boolean' as const, description: '是否已排队。' },
      }),
      render: (_args, value) => [{ type: 'text', text: textOf(value as never) }],
    ...
    execute: async (_args, exec) => {
      ...
      return outcomeResult(
        outcome,
        '已回到上一个文件。',
        '回到上一个文件的操作已排队，将在 Sidebar 可见时自动应用。',
        { path: resolved.path },
      )
    },
  })))
...
function renderState(value: { ok: boolean; code: string; message: string; state?: Record<string, unknown> | null; connected?: boolean }): string {
  if (value.ok && value.state !== null && value.state !== undefined) {
    ...
    const files = Array.isArray(s.openedFiles) ? (s.openedFiles as string[]).join(', ') : '（无）'
    const folders = Array.isArray(s.expandedFolders) ? (s.expandedFolders as string[]).join(', ') : '（无）'
    return [
      `当前文件: ${current}`,
      `上一个文件: ${previous}`,
...
function renderListing(value: { ok: boolean; code: string; message: string; path?: string; entries?: Array<{ name: string; isDir: boolean; hidden: boolean }>; truncated?: boolean }): string {
  const lines: string[] = []
  if (!value.ok) return `[${value.code}] ${value.message}`
  lines.push(`[OK] ${value.message}`)
  if (typeof value.path === 'string') lines.push(`目录: ${value.path}`)
  if (Array.isArray(value.entries)) {
    for (const entry of value.entries) {
      const mark = entry.isDir ? '📁' : '📄'
      lines.push(`  ${mark} ${entry.name}${entry.hidden ? ' (hidden)' : ''}`)
    }
    if (value.truncated === true) lines.push('  …（条目过多，已截断）')
  }
  return lines.join('\n')
}
