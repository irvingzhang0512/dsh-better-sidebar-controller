window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function deriveSidebarState(prev, snapshot) {
			const state = snapshot.state;
			const fileContext = deriveFileContext(prev, state === void 0 ? null : activeFileOf(state));
			return {
				sessionId: snapshot.sessionId ?? null,
				sidebarVisible: state?.panelOpen ?? false,
				openedFiles: state === void 0 ? [] : openedFilesOf(state),
		...
		*/
		function findTabByPath(state, path) {
			...
			for (const float of state.floats) if (isEditorFileTab(float.tab) && float.tab.path === path) return float.tab;
		}
			...
			ctx.effect(() => {
				...
				const handleCommand = (id, command) => {
					switch (command.name) {
						case "show_sidebar": {
							...
							s.reduce((state) => state.panelOpen ? state : {
								...state,
								panelOpen: true
							});
							ack(id, {
				...
				*  session switches must not reopen/activate the anchor again). */
				const openAnchorInBackground = (snapshot) => {
					...
					const previous = snapshot.state === void 0 ? null : activeTabIdOf(snapshot.state);
					service.openTab({
						type: ANCHOR_TYPE,
						id: ANCHOR_TAB_ID,
						title: ""
