...
import { isWithin } from "./fs-tree.js";
 ...
 */
export function resolveSessionPath(cwd, target, platform = process.platform) {
 ...
 */
export async function resolveWorkspaceTarget(cwd, raw) {
    if (raw === '') {
    ...
    const [realCwd, realTarget] = await Promise.all([
        ...
        realpath(absolute).catch(error => error),
    ]);
 ...
 */
export function resolveLexicalTarget(cwd, raw, platform = process.platform) {
    if (raw === '') {
        return { ok: false, code: 'INVALID_PATH', message: '路径不能为空。' };
    }
    let absolute;
...
export async function pathKind(path) {
    const info = await stat(path).catch(error => error);
